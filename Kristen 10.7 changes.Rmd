...
```
This histogram shows balances that are a lot more evenly distributed, which will hopefully make our model more accurate.
...
summary(Salaries)
head(Salaries)
plot(Salaries, main="Salaries", col.main="#EAAA00",col="#002855")
```
...
```{r}
par(mfrow=c(1,3))
plot(salary~rank, main="Salary and Rank", col.main="#EAAA00",col="#002855")
plot(salary~discipline, main="Salary and Discipline", col.main="#002855", col="#EAAA00") 
plot(salary~sex, main="Salary and Sex", col.main="#EAAA00",col="#002855")
```
...
```{r}
par(mfrow=c(1,3))
plot(AsstProfessors$salary~AsstProfessors$sex, main="Rank and Salary (Asst Prof)", col.main="#EAAA00",col="#002855")
plot(AssocProfessors$salary~AssocProfessors$sex, main="Rank and Salary (Assoc Prof)", col.main="#002855",col="#EAAA00")
plot(Professors$salary~Professors$sex, main="Rank and Salary (ALL)", col.main="#EAAA00",col="#002855")
```
...
summary(mod5)
```
Again we see that from this model that its estimates are that in every rank, males make more in comparison to females.
...
```{r}
par(mfrow=c(1,1))
plot(salary~yrs.service, main="Salary and Years Service", col.main="#EAAA00",col="#002855")

mod7 <- lm(salary~yrs.service,data=Salaries)

par(mfrow=c(2,2))
plot(mod7,main="Rank and Salary (Asst Prof)", col.main="#002855",col="#EAAA00")
```
...
par(mfrow=c(2,2))
plot(mod8, main="log(salary) and years service transformed", col.main="#EAAA00",col="#002855")
```
...
```{r}
par(mfrow=c(1,1))
plot(mod8$fitted.values~yrs.service, main="Fitted Values and Years Service", col.main="#002855",col="#EAAA00")
```
```{r}
exp(predict(mod8,newdata=data.frame(yrs.service=5)))
exp(predict(mod8,newdata=data.frame(yrs.service=25)))
exp(predict(mod8,newdata=data.frame(yrs.service=50)))
```

Looking at the yrs.since.phd vs salary relationship:

```{r}
plot(log(salary)~yrs.since.phd, main="log(salary) and years of Service", col.main="#EAAA00",col="#002855")
```
...
par(mfrow=c(2,2))
plot(mod9, main="Salary and Years Since phd", col.main="#002855",col="#EAAA00")
```
...
par(mfrow=c(2,2))
plot(mod10, main="Salary and everything else (except sex)", col.main="#EAAA00",col="#002855")
```
...
Finally, we fitted a final model to predict salary amounts based on all of the predictor variables (except sex because we found it to be biased in our data). This model could be used as a baseline for making offers of new faculty members, however we do not think it is entirely suitable based on the information provided.